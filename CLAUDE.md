...
These docs are the source of truth for where Studio is going. They live in [`design-docs/`](./design-docs/) (moved from the repo root, 8 Aug 2026; code comments still cite them by bare filename). Read them before reaching for related tasks; they describe the architecture, the rationale, and the phased rollout. **If anyone asks "what's the plan for Studio?", these are the answer.**

- **[`STUDIO-LEARNING.md`](./design-docs/STUDIO-LEARNING.md)** — How Studio gets better with use without fine-tuning. Covers the corpus (retrieval-backed generation), Generation Source toggle (LLM / Corpus / Compare), preference loop (accept/reject/comments-on-nodes), App Brief skill, display axes (visualWeight / density / information), gaps log, Mobbin-fed seed generator. 6-phase rollout #113–#118 + Phase 0 #121.

- **[`STUDIO-CHAT.md`](./design-docs/STUDIO-CHAT.md)** — How the chat surface becomes a rich generative-UI workspace. Covers the AI SDK tool-call protocol, the 8-tool catalog (askQuestions, proposeLayouts, confirmGap, suggestRename, pickIcon, confirmDestructive, saveAsUserComponent, reviewLearnings), Vercel AI Elements adoption, voice input, inline artifacts vs canvas artifacts. 6-phase rollout (A–F) running parallel to the learning rollout.

- **[`STUDIO-THEMES.md`](./design-docs/STUDIO-THEMES.md)** — What a theme *is* and how it moves between people. The `ThemeInput` contract (deterministic, portable), the three visibility tiers (private project variants → curated share set → community catalog), remix lineage (`remixOf`), and showcase "clones" as seed content. T0–T5 rollout; T0 (variant storage, migration `0013`) is built.

- **[`STUDIO-STORAGE.md`](./design-docs/STUDIO-STORAGE.md)** — User-owned **assets** (their own images, later fonts/video/exports) as distinct from row-based project data. Supabase Storage bucket + `assets` table with RLS mirroring `project_access`, an upload/library flow, and wiring uploaded URLs into the existing `mediaUrls` channel that today only carries stock/generated fills. Signed-URL delivery for shares. S0–S4 rollout; S0 substrate + S1 browser + provenance metadata built (migration `0014`).

- **[`STUDIO-AUDIT.md`](./design-docs/STUDIO-AUDIT.md)** — The cross-cutting **activity trail**. One `events` table + one `logEvent` primitive every feature funnels through, append-only, RLS-scoped to projects. Renders "Ali created an image on {Screen} inside {Project} at {time}, using {model}". Pairs with asset provenance and the revision spine (object vs. timeline). A0–A4 rollout.

- **[`STUDIO-FILLS.md`](./design-docs/STUDIO-FILLS.md)** — The **fill model**: a background (shader / image / video / gradient / solid) is a *fill of a frame*, never a free-standing node. The `BackgroundFill` render boundary + `FillValue` shape, the Figma-style `FillPicker`, frame capability (incl. AppShell) + breadcrumb rooting, and the future grouped/searchable **token picker** (registry-abstracted for the DTCG migration) + Studio popover-panel rationalisation. F0 built; F1–F5 rollout.

- **[`STUDIO-TOKENFIELD.md`](./design-docs/STUDIO-TOKENFIELD.md)** — The inspector's **value-editing rules in plain English**: the three field states (unset / bound token / detached raw), why detached values are inline `style` (Fast Frame's precompiled CSS), display + interaction rules (chips, ghost zeros, detach/attach, closest-match menu, maths + arrow keys, units), the Custom badge + reset, the `@source inline` safelist contract, and the registry seam. Read this before touching `TokenField` or any inspector section.

- **[`STUDIO-EMBED.md`](./design-docs/STUDIO-EMBED.md)** — **`grade-embed`**: live, isolated Grade renders embeddable anywhere (personal site, blog, Webflow). Fast Frame minus editing chrome — reuses `FastIframeHost`, the share-link record, and the share view's viewport/Fit-scale math. Two modes (React `<GradeEmbed>` + hosted `embed.gradeui.com/e/<id>`), point-at-share-id payload, and the non-trivial sizing story (aspect-ratio + device-frame done; cross-origin auto-height is the new bit). Same kernel-extraction workstream. E0–E4 rollout.

- **[`STUDIO-CAPTURE.md`](./design-docs/STUDIO-CAPTURE.md)** — One **capture primitive**, three consumers: grid posters (the memory-ceiling fix when a project has many screens), static HTML export, and the live embed. Capture = rendered DOM + theme var block + media map; a promote-on-view / demote-to-poster grid policy; an export packaging pass. C0–C4 rollout.

- **[`STUDIO-PERSISTENCE.md`](./design-docs/STUDIO-PERSISTENCE.md)** — When and how Studio **saves** without hammering Supabase. Dirty-tracking via a content signature (only write when content actually changed, so loading a project never re-saves over a newer edit — the bug that was silently eating manual edits), debounce + flush-on-leave, a granular `saveScreen`, and the "surface every write error, never swallow it" rule. P0–P4.

- **[`STUDIO-DIRECTOR.md`](./design-docs/STUDIO-DIRECTOR.md)** — **Record → payload → replay**: turn a live screen into a directed product demo. A JSON timeline (camera / cursor / interaction / caption / audio / video tracks) replayed over the live transcluded canvas with auto-zoom, a focus spotlight, a synthetic cursor, captions, and (later) voiceover + webcam. Live-first (editable, embeddable, pin-to-revision for a point-in-time demo), video-on-demand via Playwright. The camera timeline (`useCameraTimeline` / `ZoomPan` in the embed) is track one, shipped; the `camera-tour` playground scaffold is a working showcase. D0–D5.

- **[`STUDIO-TAGS.md`](./design-docs/STUDIO-TAGS.md)** — **Screen organisation**: flat TYPED tags with hierarchy as a VIEW (group-by = folders, faceted filtering), a per-project tag registry, and **flows as a facet** — `flow` tags make STUDIO-FLOWS' implicit goto-graph addressable: share a flow (entry screen + scoped flow map), flow-scoped goto resolution with `tagAlias` for Screen-A/Screen-B variant walkthroughs from one hub. T0–T4 rollout; T0 SHIPPED (DesignTag on Design, adapter threading, inspector Tags editor — f60db6b); NEXT UP: T1 rail views (group-by / filter / list).

- **[`STUDIO-EDITS.md`](./design-docs/STUDIO-EDITS.md)** — **Streamed AI edits (true diffs)**: iteration turns emit anchored `jsx-edit` SEARCH/REPLACE blocks instead of regenerating the full component — output tokens shrink from O(page) to O(change), each block applies the moment it seals, and the double-buffered preview morphs near-instantly. Covers the wire contract, the three-tier apply engine (`studio-edit-blocks.ts`), the three-lane draft gate (fresh-build partials / applied-edit drafts / hold-and-snap), failure chips with retry-as-regen, and the full-fence escape hatch. X0–X4 rollout.

- **[`THEME-MIGRATION.md`](./THEME-MIGRATION.md)** — **At-will theming**: the priority plan to the "magic demo" (drag hue / type ratio / density on any generated screen, everything re-themes live, retroactively). Phase A: Tailwind v4 native @theme migration (zero visual change, class-diff + screenshot gated); Phase B: wire density to `--spacing` and modular scales to `--text-*`, role ramp families; Phase C: ban theme-unreachable absolutes in generated output; Phase D: the demo.

- **[`STUDIO-BYODS.md`](./design-docs/STUDIO-BYODS.md)** — **Bring your own design system**: the `DesignSystemRegistry` contract (in `packages/studio/src/registry/`) that makes Studio's generation pipeline pluggable. Three levels (theme pack / theme + custom knowledge / BYO npm components), registry layering via `extends`, the consumer migration order, and the sandbox-origin prerequisite for uploaded bundles. B0 (contract + gradeui default registry + registry-fed system prompt) is built.

- **[`STUDIO-COMPONENTS.md`](./design-docs/STUDIO-COMPONENTS.md)** — **Shared components**: project-scoped reusable JSX modules stored in Supabase (shared_components, migration 0025), imported by screens via `"@project/components"`, live-updatable without checkins. MCP-first authoring (save/list/get/delete_shared_component), lazy cycle-guarded compilation in every renderer, Figma-style sealed-instance selection (Stage 1 shipped), read-only rail viewer. Stage 2 (edit-the-master) designed. Read this BEFORE touching shared-component resolution, the boundary wrapper, or the panel bundle.

- **[`STUDIO-CANVAS.md`](./design-docs/STUDIO-CANVAS.md)** — The **freeform AI canvas**: from "a camera pointed at one screen" to a scene graph of many objects (screens / images / media placeholders / notes) placed freeform on the existing pointer-anchored camera. Covers the `canvas_objects` model, the three render tiers (plain DOM / STUDIO-CAPTURE posters / promoted live iframes), AI placement via a `placeObjects` chat tool with camera framing, paste/drop, bounded↔infinite bounds, and the **sandbox origin split** — the security prerequisite (same-origin iframes are not a boundary) that must land before any other-people's-code surface ships. K0–K5 rollout.
