...
const config: Config = {
  // v4: bare string form (the one-element `["class"]` tuple is no longer
  // accepted by v4's Config type).
  darkMode: "class",
  content: [
  ...
  theme: {
    extend: {
      ...
      colors: {
        ...
        // Grade semantic extras — same OKLCH wrapping pattern. Each status
        // colour exposes `-soft` / `-deep` siblings, which map to the tinted
        // alert surface + readable text tokens generated by the theme
        // pipeline (lib/themes/oklch.ts#deriveAlertPair).
        success: {
