...
function EmptyPreview({
  ...
  // Per-project registry: the reference-layout gallery is gradeui JSX
  // rendered as gradeui thumbnails — wrong DS on an external project.
  // Branch to a CHILD component per registry kind: the registry can
  // flip after mount (project override lands via useSyncExternalStore),
  // and an inline early-return here skipped the gallery's hooks —
  // "Rendered fewer hooks than expected". Children remount cleanly.
  const emptyPreviewRegistry = useActiveRegistry();
  if (emptyPreviewRegistry.id !== "gradeui") {
    return (
      <ExternalTemplatesEmpty
...
function ExternalTemplatesEmpty({
  registry,
  onPickStarter,
}: {
  registry: ReturnType<typeof useActiveRegistry>;
  onPickStarter?: (scaffold: string, layoutId: string) => void;
}) {
  const externalTemplates = (registry.templates ?? []).filter(
    (t) => t.source,
  );
  return (
      ...
      <div className="mx-auto max-w-3xl px-6 py-10 md:px-10">
        <div className="flex flex-col items-center text-center">
          ...
          <p className="text-sm text-muted-foreground max-w-sm">
            …or seed this screen from one of {registry.name}&rsquo;s
            templates.
          </p>
        </div>
        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          ...
          {externalTemplates.length === 0 && (
            <p className="col-span-full text-center text-sm text-muted-foreground">
              ...
              registries/{registry.id}/templates/.
            </p>
          )}
        </div>
      </div>
...
function GradeuiStarterGallery({
  theme,
  mode,
  onPickStarter,
}: {
