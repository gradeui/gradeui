...
- `grade:fast-compile` carries an optional `modules` field — **project shared components** ({name → raw JSX module source}, from the `shared_components` table, migration 0025). The sandbox compiles each lazily and resolves `import { X } from "@project/components"` from them (see "Shared components" below). Two more optional fields (8 Aug 2026), both Fast Frame only (share/embed flows never run on Sandpack):
  - `navigation: true` marks a **flow-navigation** push (STUDIO-FLOWS goto). The sandbox treats it as a page change, not an edit: it scrolls its document to the top AFTER that compile commits (a parent-side scroll fires before the morph, and React can carry the focused goto button into the new tree, where the browser re-scrolls it into view — long forms opened mid-page), and it skips the STUDIO-EDITS changed-node flash (sibling screens share chrome, so the edit-diff heuristic would pulse titles and fields on every hop). `FastIframeHost` arms it from its `navigationKey` prop, keyed on the flow entry id, never the source, so streamed edits keep their scroll position and their flash.
  - `flashEdits: false` disables the changed-node flash wholesale. The share view and the embed pass it via the host prop of the same name: those are viewer surfaces, nothing there is ever an edit.
...
- `grade:set-safe-area` (home-screen standalone + the display's REAL safe-area insets — stamps `data-standalone="true"` and `--gds-safe-area-top/right/bottom/left` on `<html>`). The host has to measure and push these because `env(safe-area-inset-*)` evaluated INSIDE the sandbox iframe resolves against the iframe's own box and is therefore always zero, however deep the notch. Posted by `FastIframeHost` from `lib/standalone-insets.ts`, and ONLY when the top-level document is genuinely installed to a home screen, so a normal tab, a Studio tile and a capture run never see it. The gate is top-level-only on purpose: `display-mode` propagates to nested browsing contexts but `env()` does not, so an `/e/` embed iframed inside another page would otherwise report standalone with zero insets and tell the prototype to stand its own status bar down with nothing put in its place. A prototype consumes it by hiding its simulated status bar under `html[data-standalone]` and reading the vars instead of its own per-handset constants (Glint's `MobileViewport` shared component is the worked example). NOT yet mirrored to the `ext:*` external-registry path, so BYODS shares (brightlocal) never receive it.
