...
- `grade:fast-compile` carries an optional `modules` field — **project shared components** ({name → raw JSX module source}, from the `shared_components` table, migration 0025). The sandbox compiles each lazily and resolves `import { X } from "@project/components"` from them (see "Shared components" below). It also carries an optional `resetScroll: true` marker on **flow-navigation** pushes (STUDIO-FLOWS goto): the sandbox scrolls its document to the top after that compile commits, because a parent-side scroll fires before the morph and React can carry the focused goto button into the new tree, where the browser re-scrolls it into view (long forms opened mid-page). `FastIframeHost` arms the marker from its `resetScrollKey` prop (keyed on the flow entry id, never the source, so streamed edits keep their scroll position). Fast Frame only: the Sandpack agent has no equivalent because share/embed flows run exclusively on Fast Frame.
