...
- **Fast Frame** — the live Studio preview, and the share/embed/poster routes — resolves `@gradeui/ui` through the workspace symlink to **`packages/ui/dist`**, NOT from npm and NOT from `apps/docs/components/ui/*`. Both `@gradeui/ui` specifiers and any relative `./components/ui/*` a screen emits are routed to that one dist barrel (`app/fast-sandbox/page.tsx:150,205`, `lib/studio-render-core.tsx:114,145`); the only vendored exception is `Composer`, which must stay a single module instance for Lexical. So a library change reaches Studio as soon as you run `pnpm -F @gradeui/ui build` — no publish — but NOT before, since Next reads dist (`@gradeui/ui` is deliberately absent from `transpilePackages`).
- **Sandpack** is the exception that still npm-installs, pinned by the registry (`packages/studio/src/registry/gradeui.ts`), so it can be many versions behind Fast Frame. See `STUDIO.md` → "The publish-lag gotcha", which is the accurate account of both paths.
