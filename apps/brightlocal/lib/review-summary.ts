...
export function reviewSummaryFor(s: ReviewStats, isStarter: boolean): ReviewSummary {
  ...
  if (r.lowCount >= 3) {
    lines.push({
      ...
      segments: [
        t("Out of your last "), m(String(r.lastN), "neutral", "Your most recent reviews across every connected review site."), t(" reviews, "), m(String(r.lowCount), "bad", `Reviews at ${r.lowStar} stars or lower, or a Facebook thumbs down. They are at the top of Review Manager.`), t(` ${r.lowCount === 1 ? "was" : "were"} ${r.lowStar} stars or lower.`),
