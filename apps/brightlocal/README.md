...
preset, every screen and variation, the settings page and the docs.
`Alt T` on any screen opens the proposal module's layout tweaker.
`/settings` is the long-form version of the menu. `?persona=<id>` on any
link selects a persona.
...
components/               goto bridge, demo command menu
lib/
