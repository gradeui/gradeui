...
export function Wordmark({
  ...
  cut?: "champagne" | "metal";
  /** "metal" is the gold gradient (the brand mark). "current" paints
   *  the glyph in currentColor, for icon slots where it sits beside
   *  stroked icons and has to read as one of them — a filled gold
   *  glyph next to 1.5px lucide strokes reads louder and larger than
   *  its box. Monochrome also lets it inherit the row's active state. */
  tone?: "metal" | "current";
  /** "full" is the GLINT logotype; "mark" is the G glyph alone for
   *  icon rails, collapsed sidebars, and other tight chrome. */
  lockup?: "full" | "mark";
  className?: string;
  /** Passed to the <svg>. With tone="current" this is how a caller
   *  sets the paint — `style={{ color: metalSolid("gold") }}` gives the
   *  glyph the flat metal colour. */
  style?: React.CSSProperties;
}) {
  const stops = CUTS[cut] ?? CUTS.metal;
  const gradId = `glint-wm-${lockup}-${cut}`;
  /* Monochrome skips the gradient entirely — no <defs>, no paint
     server, just currentColor. */
  ...
  const defs = mono ? null : (
    <defs>
      <linearGradient
        ...
        x1="0"
        y1="0"
        x2={lockup === "mark" ? "24" : "103"}
        y2="24"
        gradientUnits="userSpaceOnUse"
      >
        {stops.map(([offset, step]) => (
          <stop
            key={offset}
            offset={offset}
  ...
  if (lockup === "mark") {
    return (
      ...
      >
        {defs}
        <path d={G_RING} fill={paint} />
        <path d={G_BAR} fill={paint} />
      </svg>
    );
  }
