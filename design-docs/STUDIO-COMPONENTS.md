...
- **Selection**: any click inside a boundary snaps to the OUTERMOST boundary (sealed instance). `SelectionPayload`/`StudioSelection` carry `boundary?: string`; the path bar shows `App › <Name>`; the inspector renders a shared-component card instead of the contract panel (which also closed a live mis-target hazard: internal-part clicks used to edit the first same-named component in screen source).
