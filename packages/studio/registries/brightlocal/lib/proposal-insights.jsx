...
function ActionWhere({ item, action, index }) {
  ...
  // PER-ACTION DEEP LINK: a link may carry `goto` ("screen:<id>"). The
  // wrapper span holds it, because Button drops unknown data-* props and
  // the goto bridge resolves from the closest [data-grade-goto]. A link
  // with no goto stays inert.
  return (
  ...
  return (
    <AccordionItem value={`action-${index}`}>
      ...
      <AccordionTrigger
        ...
        // data-hook, not dataHook: AccordionTrigger (2.27.0) has no such
        // prop and forwarded it to the DOM as an unknown attribute.
