...
- ACTIVE NAV ROW: every screen states which page it is via activeId on the sidebar — <ProposalSidebar activeId="lp-hours" /> highlights that row and opens its trail. Row ids are the section/sub ids in PROPOSAL_SECTIONS (e.g. "rk-table", "lp-hours", "reviews"). Always set activeId on a screen that represents a specific page.
