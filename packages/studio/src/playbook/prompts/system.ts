...
export function buildSystemPrompt(
  ...
  // Component-conditional stanzas: gradeui-specific guidance only renders
  // when the registry actually exposes the component it describes. With
  // GRADE_REGISTRY every gate is true, so the output stays byte-identical
  // (the B0 invariant). A registry that lacks e.g. Sortable simply loses
  // that rule — the numbering gap is harmless.
...
10. When the user asks for iterations ("make it bigger", "red instead of green"), follow the EDIT MODE instructions if they are present in this prompt; otherwise regenerate the FULL component so the preview updates in one go.${has("Section") ? `

LAYOUT PRIMITIVES — prefer these over hand-rolled flex/grid utility classes where they fit. Using the primitives keeps structure editable via the settings panel and keeps vertical/horizontal rhythm consistent. Raw Tailwind is still fine for one-offs INSIDE a section — but the PAGE SCAFFOLD below is required, not a suggestion.
  - PAGE SCAFFOLD (required): a page is an ordered stack of <Section> bands, and EVERY <Section> wraps a <Container>. The <Section> is the full-width band (pick a \`scope\` for colour/theme + \`pad\` for rhythm); the <Container> is the measure (\`maxW\` sm|md|lg|xl|prose|full; \`grid\` for a 12-col split). Put content INSIDE the Container — never directly in the Section. For an edge-to-edge band use <Container maxW="full"> — you never omit the Container. NEVER emit a raw <section className="py-…"> or a <div className="max-w-… mx-auto px-…"> page-band wrapper; <Section> + <Container> replace exactly that. Use the Section parts (SectionEyebrow / SectionTitle / SectionSubtitle / SectionDescription / SectionActions / SectionMedia) for the heading+copy+CTA+media shape. This holds for app content regions too — AppShell is only the outer chrome; the regions inside it are still <Section> → <Container>.
  ...
  - <Flex direction="…" gap="…"> when you need reverse direction, baseline alignment, or CSS defaults rather than Row's \`items-center gap-md\` starting point.` : ""}${extraRules}`;
}
