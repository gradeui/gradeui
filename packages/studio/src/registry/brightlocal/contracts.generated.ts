...
// @brightlocal/ui-components@2.27.0's dist/index.d.ts — do not
...
> = {
  ...
  "AlertDestructive": {
    ...
    "props": {
      ...
      "title": {
        ...
        "design": "knob",
        "optional": true,
        "description": "Title for the alert Optional title for the alert"
      },
      "description": {
        "kind": "unknown",
        "design": "content",
        "optional": true,
        "description": "Optional description text for the alert Description text for the alert"
      }
    }
  },
  "AlertDialog": {
  ...
  "ComboboxLoading": {
    ...
    "props": {
      "loadingText": {
        ...
        "description": "Loading announcement for assistive technologies (visually hidden). The default loading state is a spinner only — the surrounding combobox makes the context unambiguous, so visible text would be noise."
      }
    }
  },
  "ComboboxSeparator": {
  ...
  "DataTable": {
    ...
    "props": {
      ...
      "getRowClassName": {
        ...
        "design": "event",
        "optional": true,
        "description": "Compute an extra class name for a body row. Called for every data row, including expanded sub-rows (check `row.depth` / `row.getIsExpanded()`). Not called for skeleton, empty-state, or expanded detail-panel rows. Merged after the base row classes via `cn`, so a plain background class (for example `bg-red-50`) replaces the built-in expanded-zone tint."
      },
      "renderExpandedContent": {
        "kind": "unknown",
        "design": "event",
        "optional": true,
        "description": "Render a full-width detail panel below each expanded row: an extra `<tr>` with a single cell spanning all visible columns. Designed for the detail-panel pattern (`getRowCanExpand` on `useDataTable`, no `getSubRows`); when combined with `getSubRows`, the panel renders between the parent row and its sub-rows. Return `null`/`undefined` to skip the panel for a given row. Not supported by `VirtualizedDataTable`. Lazy-load expensive detail: fetch on expand (trigger the request from `onExpandedChange` on `useDataTable`) and return `<DataTableExpandedSkeleton />` while the row's data is in flight so the panel keeps a stable height instead of jumping twice."
      }
    },
    "variantDefaults": {
      "layout": "auto"
  ...
  "DataTableExpandRowButton": {
    "name": "DataTableExpandRowButton",
    "props": {
      "row": {
        "kind": "unknown",
        "design": "plumbing",
      ...
      "rowName": {
        ...
        "design": "knob",
        "optional": true,
        "description": "Human-readable name of the row, interpolated into the accessible labels so every toggle identifies its row to screen-reader users (\"Expand details for Brighton — North Laine\"). Pass the row's primary display value (e.g. `row.original.name`). Without it the labels fall back to the generic \"Expand row\" / \"Collapse row\" — fifty identical buttons — so always provide it when rows have a natural name."
      },
      "expandLabel": {
        "kind": "string",
        "design": "knob",
        "optional": true,
        "default": "Expand details for {name}\" when `rowName` is set, \"Expand row\" otherwise",
        "description": "Accessible label when the row is collapsed. Supports the `{name}` placeholder, replaced with `rowName`."
      },
      "collapseLabel": {
        "kind": "string",
        "design": "knob",
        "optional": true,
        "default": "Collapse details for {name}\" when `rowName` is set, \"Collapse row\" otherwise",
        "description": "Accessible label when the row is expanded. Supports the `{name}` placeholder, replaced with `rowName`."
      },
      "dataHook": {
        "kind": "string",
        "design": "plumbing",
        "optional": true,
        "default": "expand-row-{row.id}",
        "description": "Data hook for automated testing."
      },
      "trackingEl": {
        "kind": "string",
  ...
  "DataTableExpandedSkeleton": {
    "name": "DataTableExpandedSkeleton",
    "props": {
      "fields": {
        "kind": "number",
        "design": "knob",
        "optional": true,
        "default": 3,
        "description": "Number of field-shaped placeholder lines. Match the number of key-value fields the loaded panel will render so the row height stays stable when the content arrives."
      },
      "loadingLabel": {
        "kind": "string",
        "design": "knob",
        ...
        "description": "Screen-reader announcement while the panel content loads."
      },
      "dataHook": {
        "kind": "string",
        "design": "plumbing",
        "optional": true,
        "default": "data-table-expanded-skeleton",
        "description": "Data hook for automated testing."
      }
    }
  },
  ...
  "GlobalLayout": {
    ...
    "props": {
      ...
      "maxWidth": {
        ...
        "optional": true,
        "default": "none",
        "description": "Maximum width of the global container. Unconstrained by default so the sidebar pins to the viewport edge and content centres in the remaining space; pass a `breakpoint` token from `@brightlocal/tokens/breakpoints` to cap the whole shell."
      }
    },
    "element": "div"
  },
  ...
  "GlobalLayoutImage": {
    "name": "GlobalLayoutImage",
    "props": {
      "dataHook": {
        "kind": "string",
        "design": "plumbing",
        "optional": true,
        "default": "content-header-image",
        "description": "Data hook for automated testing"
      }
    },
    "element": "div"
  ...
  "GlobalLayoutSidebar": {
    ...
    "props": {
      ...
      "width": {
        ...
        "design": "plumbing",
        "optional": true,
        "description": "Width of the sidebar (default: 288px)"
      }
    },
    "element": "div"
  },
  "GlobalLayoutSubtitle": {
    ...
    "props": {
      "asChild": {
        ...
        "design": "knob",
        "optional": true,
        "description": "Render the subtitle as its own child element (Radix Slot) while keeping the muted description typography — use it for rich description rows (icons, rating, a status Badge) that need their own flex layout."
      },
      "dataHook": {
        "kind": "string",
        "design": "plumbing",
  ...
  "Link": {
    ...
    "props": {
      ...
      "variant": {
        ...
        "optional": true,
        "default": "inline",
        "description": "Visual style variant. `inline` renders a standard text link; `inherit` renders an always-underlined link that takes the surrounding text colour (for links inside body copy, alerts and muted text); `primary` renders the brand CTA treatment (matches `Button variant=\"primary\"`); `filled`, `outline` and `ghost` render button-styled treatments for inline actions and high-priority navigation; `icon` renders a circular icon-only link (provide an `aria-label` since there is no text content)."
      },
      "trackingEl": {
        "kind": "string",
        "design": "knob",
  ...
  "Progress": {
    ...
    "props": {
      "label": {
        ...
        "design": "knob",
        "optional": true,
        "description": "Optional visible label text displayed to the left of the progress bar. Rendered inside a fixed-width column (see `labelWidth`); longer labels wrap onto further lines while the bar and value stay in their columns."
      },
      "color": {
        "kind": "enum",
        "values": [
      ...
      "size": {
        ...
        "optional": true,
        "default": "sm",
        "description": "Row size: `sm` = 14px text with an 8px bar, `md` = 16px text with a 12px bar."
      },
      "ariaLabel": {
        "kind": "string",
        "design": "plumbing",
      ...
      "indeterminate": {
        ...
        "optional": true,
        "default": false,
        "description": "Runs the bar as an indeterminate sweep: a short pill travelling the track for \"running, and we cannot say how far\". Use when work is genuinely underway but the backend reports no percentage — never as a stand-in for a number you could have fetched. `value` is ignored while this is set, the numeric readout is suppressed (there is nothing to report), and the bar drops `aria-valuenow` so assistive technology announces it as indeterminate rather than as 0%."
      },
      "labelWidth": {
        "kind": "enum",
        "values": [
        ...
        "optional": true,
        "default": "40%",
        "description": "Width of the label column: `\"20%\"` … `\"50%\"` of the row in 10% steps, or `\"auto\"`. Every row that uses the same percentage starts its bar at the same x-position, so stacked rows inside a card line up — pick the step that keeps the labels in that card to one or two lines. Use `\"auto\"` for a single row that isn't aligned with others: the label hugs its text (capped at 40% of the row) and the bar starts right after it."
      },
      "showValue": {
        "kind": "boolean",
        "design": "knob",
      ...
      "indicatorClassName": {
        ...
        "design": "knob",
        "optional": true,
        "description": "Custom class name for the indicator (progress fill) Use this to customize the color, e.g., \"bg-chart-red\""
      },
      "statusDescription": {
        "kind": "unknown",
        "design": "content",
        "optional": true,
        "description": "A line describing what the run is doing right now — the step, not the number. Renders on its own line above the bar's row, so `label` stays on the left and the value on the right. It takes the row's `size`, so it reads as a peer of the value rather than a second scale. It is a polite live region, and it never becomes the bar's accessible name (`ariaLabel` still owns that) — which is what makes it the right slot for a status that moves, where `label` is not."
      },
      "getValueLabel": {
        "kind": "unknown",
        "design": "event",
  ...
  "ProgressScreen": {
    ...
    "props": {
      ...
      "illustration": {
        ...
        "design": "content",
        "optional": true,
        "description": "The tool's illustration, shown above the text. Decorative — mark it `aria-hidden`; the heading and description carry the state on their own. The determinate state has no default: pass the illustration that belongs to the thing being generated. The indeterminate state falls back to the cel loop, which is doing the work the missing progress bar cannot — pass an illustration there only if you mean to trade that motion away."
      },
      "title": {
        "kind": "unknown",
        "design": "content",
        "description": "Headline naming what is being generated, in the consumer's own words."
      },
      "description": {
        "kind": "unknown",
        "design": "content",
        "optional": true,
        "description": "Supporting line beneath the heading — the typical duration and permission to leave. A polite live region, so it may change as the run moves on."
      },
      "value": {
        "kind": "number",
        "design": "knob",
        "optional": true,
        "description": "Real progress reported by the backend. Leave it unset and the block shows the cel loop with no bar at all — never invent a percentage to fill one."
      },
      "max": {
        "kind": "number",
        "design": "knob",
        "optional": true,
        "default": 100,
        "description": "Maximum progress value, the denominator for `value`."
      },
      "progressUnit": {
        "kind": "string",
        "design": "knob",
        "optional": true,
        "default": "%",
        "description": "Unit for the progress readout. `\"%\"` renders percentage form (`45%`); any other unit renders fraction form (`3 / 5 steps`). Determinate state only."
      },
      "showProgressValue": {
        "kind": "boolean",
        "design": "knob",
        "optional": true,
        "default": true,
        "description": "Whether the determinate bar shows its numeric readout."
      },
      "progressStatusDescription": {
        "kind": "unknown",
        "design": "content",
        "optional": true,
        "description": "The step the run is on right now, shown on its own line above the bar. Determinate state only — it belongs to Progress, so it goes where the bar goes. On the indeterminate state, put anything the run can say about itself in `description`."
      },
      "progressLabel": {
        "kind": "string",
        "design": "knob",
        ...
        "description": "Accessible label for the progress bar. Determinate state only."
      },
      "trackingEl": {
        "kind": "string",
        "design": "knob",
  ...
  "RouteProgress": {
    ...
    "props": {
      ...
      "active": {
        "kind": "boolean",
        "design": "knob",
        "description": "Whether a full route navigation is in flight. Derive it from the router rather than from a click handler — that is what gives the remaining pattern guarantees for free: browser back and forward run the bar too, and in-page changes (tabs, filter changes, pagination refetches, modals) never touch it, because none of them are route navigations. ```tsx const navigating = useRouterState({ select: (state) => state.status === \"pending\", }); <RouteProgress active={navigating} dataHook=\"route-progress\" /> ``` Prefer `status === \"pending\"` over `isLoading`: `isLoading` clears as soon as the route's loaders resolve, whereas `status` returns to `\"idle\"` only once `isLoading`, `isTransitioning` and `hasPendingMatches` are all false — so it also covers the React transition that commits the new route. And always pass `select`; `useRouterState()` with no selector re-renders the whole subtree on any router state change. **Router pending alone only means \"the page is here\", not \"the page is ready.\"** It is the whole answer when the route declares its route-critical data in a loader — with TanStack Router, `loader: () => queryClient.ensureQueryData(...)` keeps `status` pending until that data has landed, which is exactly the signal this bar wants. Where components instead fetch on mount, the router goes idle as soon as the route commits and the bar finishes while skeletons are still rendering. Add the destination's own queries — **scoped to the route**, never a global `useIsFetching()`, which a polling or failing section would hold open forever: ```tsx const navigating = useRouterState({ select: (s) => s.status === \"pending\" }); const loadingPage = useIsFetching({ queryKey: [\"location\", locationId] }) > 0; <RouteProgress active={navigating || loadingPage} dataHook=\"route-progress\" /> ``` Handing off between two such signals is safe: the bar debounces the falling edge, so it stays continuous through the gap instead of sweeping and restarting. The rising edge is never debounced. Aim at *route-critical* content, not every query on the page. A below-the-fold widget that is slow, failing, or polling must keep its own skeleton and let the bar finish — a bar still running on a usable page reads as a hang."
      },
      "trackingEl": {
        "kind": "string",
        "design": "knob",
      ...
      "navigatingLabel": {
        ...
        "description": "Announced politely to screen readers while `active` is true. The bar itself is decorative — its fill is a trickle, not measured progress, so it exposes no `progressbar` role or `aria-valuenow`."
      }
    },
    "element": "div"
  },
  ...
  "SheetContent": {
    ...
    "props": {
      ...
      "size": {
        ...
        "optional": true,
        "default": "default",
        "description": "Width preset for side panels. `sm` hugs the content between 288 and 320px; `default` keeps the classic 75%-up-to-384px drawer width. Ignored for top/bottom sheets."
      },
      "side": {
        "kind": "enum",
        "values": [
  ...
  "Spinner": {
    ...
    "props": {
      "dataHook": {
        ...
        "design": "plumbing",
        "optional": true,
        "description": "Data hook for automated testing (optional for Spinner as it's purely presentational)"
      },
      "trackingEl": {
        "kind": "string",
        "design": "knob",
      ...
      "size": {
        ...
        "optional": true,
        "default": "md",
        "description": "Spinner size: sm 14 (chips, cells, nav items), md 16, lg 20 (standing alone)."
      },
      "loadingLabel": {
        "kind": "string",
        "design": "knob",
        "optional": true,
        "default": "undefined",
        "description": "Accessible label announced to screen readers (`role=\"status\"`). When omitted the spinner is decorative (`aria-hidden`) — omit inside a control that already announces busy (e.g. a Button with `aria-busy`)."
      },
      "absoluteStrokeWidth": {
        "kind": "boolean",
        "design": "knob",
