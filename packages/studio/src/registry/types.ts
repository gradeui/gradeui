...
export interface RegistryRuntime {
  ...
  previewCss?: string;
  /** Shared user-land component modules (STUDIO-FLOWS M0): import
   *  specifier → JSX/TSX SOURCE. Screens import from the specifier
   *  ("@brightlocal/proposal") instead of carrying in-file copies of
   *  shell/nav/card components. The external sandbox compiles + registers
   *  each module at boot, before any screen loads; chat-sandpack ships
   *  the same source as a file in the Sandpack map with the import
   *  aliased to it. Authored in registries/<id>/lib/*.jsx, generated by
   *  scripts/generate-registry-lib.mjs. Plain strings — serialisable. */
  libModules?: Readonly<Record<string, string>>;
}
