...
```tsx
import { Button, Card, CardContent } from "@grade/ui";
...
export default {
  ...
  content: [
    "./app/**/*.{ts,tsx,mdx}",
    "./node_modules/@grade/ui/dist/**/*.{js,mjs}",
  ],
};
```
...
`@grade/ui` ships an OKLCH-based theme generator. Wrap your app in `GradeThemeProvider` (currently still named `RampThemeProvider` pending rename — see upstream TODO) to get runtime theme switching.
