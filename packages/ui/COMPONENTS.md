...
| ~~FaqBlock~~ | `faq-block.tsx` | composed Accordion section | ❌ | **Deprecated — do not emit.** Too specific to be a primitive; compose `<Section>` + `<Accordion>` instead. Not allowlisted / not in the `@gradeui/ui` barrel; slated for removal from code. |
