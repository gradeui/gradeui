...
export const FieldContract = contract({
  ...
  styleDefaults: {"FieldSet":"flex flex-col gap-6","FieldLegend":"mb-3 font-medium","FieldGroup":"group/field-group @container/field-group flex w-full flex-col gap-7 data-[slot=checkbox-group]:gap-3 [&>[data-slot=field-group]]:gap-4","FieldTrailing":"flex shrink-0 items-center gap-2","FieldContent":"group/field-content flex flex-1 flex-col gap-1.5 leading-snug","FieldLabel":"group/field-label peer/field-label flex w-fit gap-2 leading-snug group-data-[disabled=true]/field:opacity-50","FieldTitle":"flex w-fit items-center gap-2 text-sm leading-snug font-medium group-data-[disabled=true]/field:opacity-50","FieldDescription":"text-sm leading-normal font-normal text-muted-foreground group-has-[[data-orientation=horizontal]]/field:text-balance","FieldSeparator":"relative -my-2 h-5 text-sm group-data-[variant=outline]/field-group:-mb-2","FieldError":"text-destructive text-sm font-normal"},
  props: {
  "orientation": {
