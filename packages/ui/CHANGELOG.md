...
- fa91f11: Export `SectionBlock` (and `sectionBlockVariants` / `SectionBlockProps`) from the package barrel. The component shipped in 4.0.0 but was never re-exported from `lib/index.ts`, so `import { SectionBlock } from "@gradeui/ui"` resolved to `undefined` for consumers.

## 4.0.0

### Major Changes

- 2e0464c: **Breaking:** `Composer` (and `ComposerReply` + its types) now ship on a
  ...
  re-exported from the main barrel, _any_ consumer, even one importing only
...
- 2e0464c: Two surface-consistency fixes found while dogfooding in the Astro sites spike:
